...
const Main = ({ setCurentState }) => {
    ...
    // Menu Penjualan menampilkan status lengkap untuk semua role (Owner & Kasir).
    // Status ringkas (4 status) hanya dipakai di Menu Approval Penjualan.
    const STATUS_LABEL = {
        'SELESAI': 'Selesai',
    ...
    const STATUS_OPTIONS = STATUS_OPTIONS_LENGKAP;
    ...
    const fetchData = async (page = 1, pageSize = 10, filters = {}) => {
        ...
        try {
            ...
            if (filters.status) {
                params.append('approval_status', filters.status === "CETAK KWINTANSI" ? "DISETUJUI" : filters.status);
                params.append('status', filters.status === "CETAK KWINTANSI" ? "DISETUJUI" : filters.status);
            }
